...
def my_bilateral(src, msize, sigma, sigma_r, pad_type='zero'):
    ...
    rad = msize // 2
    src_pad = my_padding(src, (rad, rad))
    (h, w) = src_pad.shape
    dst = np.zeros(src_pad.shape)

    for i in range(rad,h-rad):
        print('\r%d / %d ...' %(i,h), end="")
        for j in range(rad,w-rad):
            pixel_gaus = np.exp(-((src_pad[i-rad:i+rad+1, j-rad:j+rad+1] - src_pad[i][j])**2)/(2*sigma_r**2))
            space_gaus = 1 / (2 * np.pi * sigma**2) * np.exp(-((x**2 + y**2)/(2 * sigma**2)))
            mask = np.multiply(pixel_gaus, space_gaus)
            mask /= np.sum(mask)
            dst[i][j] = np.sum(np.multiply(mask, src_pad[i-rad:i+rad+1, j-rad:j+rad+1]))
