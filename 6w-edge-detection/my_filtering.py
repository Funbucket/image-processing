...
def my_filtering(src, mask, pad_type='zero'):
    ...
    src_pad = my_padding(src, (m_h//2, m_w//2), pad_type)  # padding 처리된 image matrix
    dst = np.zeros((h, w))
    ...
    for i in range(w):
        for j in range(h):
            val = np.sum(src_pad[i:i + m_h, j:j + m_w] * mask)
            dst[i, j] = val
...
def get_average_mask(fshape):
    ...
    (h, w) = fshape
    mask = np.ones((h, w))  # 입력한 크기의 모든 원소가 1 인 matrix 생성
